...
// Optimal Solution - Uses a segment tree with prefix, suffix, and remainder-frequency states to support dynamic updates and product-modulo range queries in O(k² log n).
/* “I use a segment tree because the array supports point updates followed by range queries. Each node stores the product remainder of the entire segment, the number of subarrays for every remainder, 
and the remainder frequencies of all prefixes and suffixes. When merging two nodes, subarrays are either completely inside the left segment, completely inside the right segment, or cross the boundary. 
For crossing subarrays, I combine every suffix remainder from the left with every prefix remainder from the right. This allows each node to contain all information required for future merges.” */
