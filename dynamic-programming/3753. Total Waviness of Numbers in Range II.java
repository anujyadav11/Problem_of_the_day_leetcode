/*********************************************** JAVA - I didn't solve this question this one is so hard for me(reference from kernel queen youtube channed)**************************************************/

// Optimal Solution - Computes total digit waviness in a range using digit DP with tight-bound tracking, counting peak and valley contributions across all valid number completions.
/*  "Digit DP is the pattern for 'count/sum some property across all numbers in [L, R]' — use f(R) - f(L-1). The state captures everything needed: position, 
    tight constraint, previous two digits for wave detection, and leading zero flag. Multiplying sub[0] * wave propagates the current wave's contribution to all valid completions below." */
